...
descarga_ultima_base <- function(base = 'EPH_usu_2_Trim_2017_txt.zip', individual = FALSE, hogar = FALSE){
  ...
  if (individual) {
    ...
    source("C:/Users/Ptiscornia/Carpeta de Trabajo/Herramientas/Procesamiento/etiquetas_b_usu_ind_eph.r")
    
  }
  if (hogar) {
    base_hogar <<- ?read.table(unz(temp,base_hogar_name), sep=";", dec=",", header = TRUE, fill = TRUE)
    source("C:/Users/Ptiscornia/Carpeta de Trabajo/Herramientas/Procesamiento/etiquetas_b_usu_HOG_eph_1t_18.r")
  }
  unlink(temp)
