...
Lamentablemente al no poseer una API (aplication program interface) de INDEC, el proceso de recuperación de la información debe atravesar por las bases completas. Eso implica un costo desprocionado en términos de tiempo y memoria, y por ello no se incorpora como funcionalidad del paquete. Pero en caso de ser necesario construir una serie, se podría realizar de la siguiente manera:

- crear una nueva función que descargue la información y haga un select
- mapearla sobre todos los periodos
...
df <- df %>% 
  ...
  summarise(indicador = sum(pondcat_ocup==2,na.rm = T)/sum(estado==1,na.rm = T)) 
df
```
